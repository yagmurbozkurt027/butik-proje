...
export async function POST(request) {
  try {
      ...
      welcome: {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Butik Proje ailesine katıldığınız için teşekkür ederiz.</p>
            <p>Hesabınızı aktifleştirmek için <a href="${data.activationLink}" style="color: #3b82f6;">buraya tıklayın</a>.</p>
            <hr style="margin: 20px 0;">
            <p style="color: #666; font-size: 12px;">Bu email Butik Proje tarafından gönderilmiştir.</p>
          </div>
        `
      },
      'password-reset': {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/email/send`, {
      method: 'POST',
      headers: {
