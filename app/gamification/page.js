...
export default function GamificationPage() {
  ...
  const fetchLeaderboard = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/gamification/leaderboard');
      const data = await response.json();
  ...
  const fetchBadges = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/gamification/${userId}/badges`);
      const data = await response.json();
  ...
  const updateTaskProgress = async (taskId, progress) => {
    try {
      const response = await fetch(`http://localhost:3001/api/gamification/${userId}/tasks`, {
        method: 'PUT',
        headers: {
