...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (userId) {
        try {
          const res = await fetch(`http://localhost:3001/api/users/profile/${userId}`);
          if (!res.ok) {
  ...
  const handleSave = async (e) => {
    ...
    const userId = localStorage.getItem("userId");
    try {
      const res = await fetch("http://localhost:3001/api/users/setup-2fa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const userId = localStorage.getItem("userId");
    try {
      const res = await fetch("http://localhost:3001/api/users/verify-2fa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const userId = localStorage.getItem("userId");
    try {
      const res = await fetch(`http://localhost:3001/api/users/login-history/${userId}`);
      const data = await res.json();
      if (res.ok) {
    ...
    try {
      const res = await fetch("http://localhost:3001/api/users/check-password-strength", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded shadow text-gray-900 dark:text-white">
      ...
      <div className="flex flex-col items-center mb-6">
        <img
          ...
          className="w-24 h-24 rounded-full object-cover mb-2 border border-gray-300 dark:border-gray-600"
        />
        <input
          type="file"
          accept="image/*"
